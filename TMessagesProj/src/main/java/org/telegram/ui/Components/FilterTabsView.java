...
public class FilterTabsView extends FrameLayout {
    ...
    public class Tab {
        ...
        public Tab(int i, String t, String e) {
            id = i;
            title = FoxConfig.tabMode != FoxConfig.TAB_TYPE_ICON ? t:"";
            realTitle = t;
            emoticon = e;
        }
        ...
        public boolean setTitle(String newTitle) {
            ...
            title = FoxConfig.tabMode != FoxConfig.TAB_TYPE_ICON ? newTitle:"";
            realTitle = newTitle;
            return true;
        }
    }
    ...
    public class TabView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            if (FoxConfig.tabMode != FoxConfig.TAB_TYPE_ICON) {
            ...
            int iconTab = 0;
            // TAB ICON
            if (FoxConfig.tabMode != FoxConfig.TAB_TYPE_TEXT) {
            ...
            if (animateCounterEnter || counterText != null || showRemove && (isEditing || editingStartAnimationProgress != 0)) {
                ...
                int textSpace = FoxConfig.tabMode != FoxConfig.TAB_TYPE_ICON ? AndroidUtilities.dp(6) : 0;
    ...
    public FilterTabsView(Context context) {
        ...
        listView.setOnItemClickListener((view, position, x, y) -> {
            ...
            if (isEditing) {
                if (position != 0 || FoxConfig.hideAllTab) {
                    int side = AndroidUtilities.dp(6);
                    if (tabView.rect.left - side < x && tabView.rect.right + side > x) {
    ...
    public void checkTabsCounter() {
        ...
        for (int a = 0, N = tabs.size(); a < N; a++) {
            ...
            if (oldWidth != width || invalidated) {
                ...
                requestLayout();
                allTabsWidth = 0;
                if (!FoxConfig.hideAllTab) findDefaultTab().setTitle(LocaleController.getString("FilterAllChats", R.string.FilterAllChats));
                for (int b = 0; b < N; b++) {
...
//                notifyItemMoved(i, i + 1);
    ...
    public class TouchHelperCallback extends ItemTouchHelper.Callback {
        ...
        public int getMovementFlags(RecyclerView recyclerView, RecyclerView.ViewHolder viewHolder) {
            if (!FoxConfig.hideAllTab && MessagesController.getInstance(UserConfig.selectedAccount).premiumFeaturesBlocked() && (!isEditing || (viewHolder.getAdapterPosition() == 0 && tabs.get(0).isDefault && !UserConfig.getInstance(UserConfig.selectedAccount).isPremium()))) {
        ...
        public boolean onMove(RecyclerView recyclerView, RecyclerView.ViewHolder source, RecyclerView.ViewHolder target) {
            if (!FoxConfig.hideAllTab && MessagesController.getInstance(UserConfig.selectedAccount).premiumFeaturesBlocked() && ((source.getAdapterPosition() == 0 || target.getAdapterPosition() == 0) && !UserConfig.getInstance(UserConfig.selectedAccount).isPremium())) {
                return false;
            }
