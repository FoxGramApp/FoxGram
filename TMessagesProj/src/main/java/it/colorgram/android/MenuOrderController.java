...
public class MenuOrderController {
    ...
    public static void loadConfig() {
        synchronized (sync) {
            ...
            if (ColorConfig.drawerItems.isEmpty()) {
                loadDefaultItems();
            }
            configLoaded = true;
        }
    ...
    public static void resetToDefaultPosition() {
        if (ColorConfig.drawerItems.isPresent()) {
            ColorConfig.drawerItems.get().clear();
            loadDefaultItems();
        }
    }
    ...
    private static int getArrayPosition(String id, int startFrom) {
        if (ColorConfig.drawerItems.isPresent()) {
            for (int i = startFrom; i < ColorConfig.drawerItems.get().size() && startFrom >= 0; i++) {
                if (ColorConfig.drawerItems.get().get(i).equals(id)) {
                    return i;
                }
            }
        }
    ...
    public static void changePosition(int oldPosition, int newPosition) {
        if (ColorConfig.drawerItems.isPresent()) {
            ColorConfig.drawerItems.get().move(oldPosition, newPosition);
            ColorConfig.applyDrawerItems();
        }
    }
    ...
    public static ArrayList<EditableMenuItem> getMenuItemsEditable() {
        ...
        if (ColorConfig.drawerItems.isPresent()) {
            for (String id : ColorConfig.drawerItems.get()) {
    ...
    private static void addAsFirst(String id) {
        if (ColorConfig.drawerItems.isPresent()) {
            ColorConfig.drawerItems.get().add(0, id);
            ColorConfig.applyDrawerItems();
        }
    }
    ...
    public static void removeItem(int position) {
        if (ColorConfig.drawerItems.isPresent()) {
            ColorConfig.drawerItems.get().remove(position);
            ColorConfig.applyDrawerItems();
        }
    }
