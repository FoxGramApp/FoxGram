...
public class AutoTranslateConfig {
    ...
    public static boolean isAutoTranslateEnabled(long dialogId, int topicId) {
        if (hasAutoTranslateConfig(dialogId, topicId)) {
            return preferences.getBoolean(getExceptionsKey(dialogId, topicId), ColorConfig.autoTranslate);
        } else {
            return preferences.getBoolean(getExceptionsKey(dialogId, 0), ColorConfig.autoTranslate);
        }
    }
