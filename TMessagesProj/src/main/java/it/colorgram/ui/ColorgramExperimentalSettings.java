...
public class ColorgramExperimentalSettings extends BaseSettingsActivity {
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        if (position == betterAudioCallRow) {
            ColorConfig.toggleBetterAudioQuality();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColorConfig.betterAudioQuality);
            }
            ...
            AlertController.show(types, LocaleController.getString("MaxRecentStickers", R.string.MaxRecentStickers), types.indexOf(String.valueOf(ColorConfig.maxRecentStickers)), context, i -> {
                ColorConfig.setMaxRecentStickers(Integer.parseInt(types.get(i)));
            ...
            ColorConfig.toggleUploadSpeedBoost();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColorConfig.uploadSpeedBoost);
            }
        } else if (position == sendLargePhotosRow) {
            ColorConfig.toggleSendLargePhotos();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColorConfig.sendLargePhotos);
            }
        } else if (position == reduceCameraXLatency) {
            ColorConfig.toggleReduceCameraXLatency();
            if (view instanceof TextCheckCell) {
                ((TextCheckCell) view).setChecked(ColorConfig.reduceCameraXLatency);
            }
        }
    }

    private void toggleExperimentalMode(TextCheckCell textCheckCell) {
        ColorConfig.toggleDevOpt();
        boolean isEnabled = ColorConfig.isDevOptEnabled();
    ...
    protected void updateRowsId() {
        ...
        if (ColorConfig.isDevOptEnabled()) {
        ...
        protected void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position, boolean partial) {
            switch (ViewType.fromInt(holder.getItemViewType())) {
                case SWITCH:
                    ...
                    if (position == betterAudioCallRow) {
                        textCheckCell.setTextAndCheck(LocaleController.getString("MediaStreamVoip", R.string.MediaStreamVoip), ColorConfig.betterAudioQuality, true);
                    } else if (position == checkBoxExperimentalRow) {
                        boolean isEnabled = ColorConfig.isDevOptEnabled();
                        ...
                        textCheckCell.setTextAndCheck(LocaleController.getString("FasterUploadSpeed", R.string.FasterUploadSpeed), ColorConfig.uploadSpeedBoost, false);
                    } else if (position == sendLargePhotosRow) {
                        textCheckCell.setTextAndValueAndCheck(LocaleController.getString("HRPhotos", R.string.HRPhotos), LocaleController.getString("HRPhotosDesc", R.string.HRPhotosDesc), ColorConfig.sendLargePhotos, true, true);
                    } else if (position == reduceCameraXLatency) {
                        textCheckCell.setTextAndValueAndCheck(LocaleController.getString("ZeroShutterLag", R.string.ZeroShutterLag), LocaleController.getString("ZeroShutterLagDesc", R.string.ZeroShutterLagDesc), ColorConfig.reduceCameraXLatency, true, true);
                    }
                    break;
                ...
                case SETTINGS:
                    ...
                    if (position == maxRecentStickersRow) {
                        textCell.setTextAndValue(LocaleController.getString("MaxRecentStickers", R.string.MaxRecentStickers), String.valueOf(ColorConfig.maxRecentStickers), partial, true);
                    }
                    break;
            }
        }
        ...
        protected View onCreateViewHolder(ViewType viewType) {
            ...
            switch (viewType) {
                ...
                case SLIDE_CHOOSE:
                    ...
                    types.add(ColorConfig.DOWNLOAD_BOOST_DEFAULT);
                    arrayList.add(LocaleController.getString("DownloadSpeedFast", R.string.DownloadSpeedFast));
                    types.add(ColorConfig.DOWNLOAD_BOOST_FAST);
                    arrayList.add(LocaleController.getString("DownloadSpeedExtreme", R.string.DownloadSpeedExtreme));
                    types.add(ColorConfig.DOWNLOAD_BOOST_EXTREME);
                    slideChooseView.setCallback(index -> ColorConfig.setDownloadSpeedBoost(types.get(index)));
                    slideChooseView.setOptions(types.indexOf(ColorConfig.downloadSpeedBoost), arrayList.toArray(new String[0]));
