...
import it.foxgram.ui.Cells.EditTopic;

public class LockedChatsGroupInfo extends BaseSettingsActivity implements NotificationCenter.NotificationCenterDelegate {
    ...
    public LockedChatsGroupInfo(TLRPC.Chat chat, EditExceptionDelegate delegate) {
        this(chat, false, delegate);
    }

    public LockedChatsGroupInfo(TLRPC.Chat chat, boolean isAllow) {
        this(chat, isAllow, null);
    }

    public LockedChatsGroupInfo(TLRPC.Chat chat, boolean isAllow, EditExceptionDelegate delegate) {
        this.chat = chat;
        ...
        boolean allowed = LockConfig.getExceptionsById(true, -chat.id);
        boolean disabled = LockConfig.getExceptionsById(false, -chat.id);
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        ...
        if (position >= startTopicsRow && position < endTopicsRow) {
            ...
            LockConfig.setEnabled(-chat.id, topic.id, !LockConfig.isChatLock(-chat.id, topic.id));
            ...
            builder.setTitle(LocaleController.getString("NotificationsDeleteAllLocks", R.string.NotificationsDeleteAllExceptionTitle));
            builder.setMessage(LocaleController.formatString("NotificationsDeleteAllLocksAlert", R.string.NotificationsDeleteAllExceptionAlert, chat.title));
            builder.setPositiveButton(LocaleController.getString("Delete", R.string.Delete), (dialogInterface, i) -> {
                LockConfig.removeGroupException(-chat.id);
                if (editExceptionDelegate == null) {
                    if (LockConfig.getExceptions(isAllow).isEmpty()) {
                        hideLastFragment();
                    }
            ...
            switch (ViewType.fromInt(holder.getItemViewType())) {
                ...
                case HEADER:
                    ...
                    if (position == headerRow) {
                        headerCell.setText(LocaleController.getString("EditTopicsLock", R.string.EditTopicsLock));
                    }
                    break;
                case EDIT_TOPIC:
                    ...
                    topicCell.setChecked(LockConfig.isChatLock(-chat.id, topic.id), partial);
                    break;
                case TEXT_HINT_WITH_PADDING:
                    ...
                    if (position == topicExceptionHintRow) {
                        hintCell.setText(LocaleController.getString("EditTopicsLockHint", R.string.EditTopicsLockHint));
                    }
                    break;
                case SETTINGS:
