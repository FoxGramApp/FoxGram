...
public class LockConfig {
    ...
    public static boolean isChatLock(long dialogId, int topicId) {
        if (hasChatLockConfig(dialogId, topicId)) {
            return preferences.getBoolean(getExceptionsKey(dialogId, topicId), FoxConfig.lockedChats);
        } else {
            return preferences.getBoolean(getExceptionsKey(dialogId, 0), FoxConfig.lockedChats);
        }
    }

    public static boolean hasChatLockConfig(long dialogId, int topicId) {
    ...
    public static boolean isDefault(long dialogId, int topicId) {
        return !hasChatLockConfig(dialogId, topicId);
    }
    ...
    private static boolean isAllTopicEnabledOrDisabled(long dialogId, boolean enabled) {
        ...
        if (topics != null) {
            return topics.stream().allMatch(t -> isChatLock(dialogId, t.id) == enabled);
        } else {
            return true;
        }
    }
    ...
    public static boolean isLastTopicAvailable(long dialogId, int topicId, boolean enabled) {
        List<TLRPC.TL_forumTopic> topics = MessagesController.getInstance(UserConfig.selectedAccount).getTopicsController().getTopics(-dialogId);
        if (topics != null) {
            return topics.stream().filter(t -> t.id != topicId).anyMatch(t -> isChatLock(dialogId, t.id) == enabled);
        } else {
            return false;
        }
    }
    ...
    public static class LockedChatsException {
        public final long dialogId;
        public final int topicId;
        public final boolean isAllow;
        public TLObject chat;

        public LockedChatsException(long dialogId, int topicId, boolean isAllow) {
    ...
    public static List<LockConfig.LockedChatsException> getExceptions(boolean isAllow) {
