/*
 * This is the source code of foxgram for Android v. 1.4.x.
 * It is licensed under GNU GPL v. 2 or later.
 * You should have received a copy of the license in this archive (see LICENSE).
 *
 * Copyright Laky64, 2021-2022.
 */
package it.foxgram.android.camera;
...
public class CameraXUtils {
    ...
    public static void loadSuggestedResolution() {
        ...
        getAvailableVideoSizes().values().stream()
                ...
                .ifPresent(height -> {
                    ...
                    if (FoxConfig.cameraResolution == -1 || FoxConfig.cameraResolution > max || FoxConfig.cameraResolution < min) {
                        FoxConfig.saveCameraResolution(height);
                    }
                });
    }
    ...
    public static Size getPreviewBestSize() {
        ...
        return getAvailableVideoSizes().values().stream()
                .filter(size -> size.getHeight() <= FoxConfig.cameraResolution && size.getHeight() <= suggestedRes)
    ...
    public static Quality getVideoQuality() {
        return getAvailableVideoSizes().entrySet().stream()
                .filter(entry -> entry.getValue().getHeight() == FoxConfig.cameraResolution)
    ...
    private static int getSuggestedResolution(boolean isPreview) {
        ...
        switch (SharedConfig.getDevicePerformanceClass()) {
            ...
            default:
                suggestedRes = FoxConfig.useCameraXOptimizedMode && isPreview ? 1080 : 2160;
                break;
        }
        return suggestedRes;
    }
